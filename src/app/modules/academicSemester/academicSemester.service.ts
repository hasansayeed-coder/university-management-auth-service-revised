...
import { academicSemesterSearchableFields, academicSemesterTitleMapper } from "./academicSemester.Constant";
...
const updateSemester = async(id : string , payload : Partial<IAcademicSemester>) : Promise<IAcademicSemester | null> => {
    ...
    if(payload.title && payload.code && academicSemesterTitleMapper[payload.title] !== payload.code){
        throw new ApiError(httpStatus.BAD_REQUEST , 'Invalid Semester Code') ;
    }
